...
namespace Gaia
{
    ...
    public class GaiaSessionManager : MonoBehaviour
    {
        ...
        public float m_genChanceOfMesas = 0f;
        ...
        public void AddResource(GaiaResource resource)
        {
            ...
            if (resource != null)
            {
                if (!m_session.m_resources.ContainsKey(resource.m_resourcesID))
                {
                    ...
                    so.m_content = File.ReadAllBytes(so.m_fileName);
                    if (so.m_content != null && so.m_content.GetLength(0) > 0)
                    {
                        m_session.m_resources.Add(resource.m_resourcesID, so);
                        SaveSession();
                    }
                    #endif
                }
        ...
        public void AddDefaults(GaiaDefaults defaults)
        {
            ...
            if (defaults != null)
            {
                ...
                m_session.m_defaults.m_content = File.ReadAllBytes(m_session.m_defaults.m_fileName);
                SaveSession();
                #endif
            }
        }
        ...
        private void PositionStamp(Bounds bounds, Stamper stamper, GaiaConstants.FeatureType stampType)
        {
            ...
            //float stampRange = 1f;
            //float fullWidth = stamper.m_width;
            float fullHeight = stamper.m_height;
        ...
        GameObject FindOrCreateObject(GaiaOperation operation)
        {
            if (operation.m_generatedByType == "Gaia.Stamper")
            {
                ...
                {
                    if (stampers[stampIdx].m_stampID == operation.m_generatedByID)
                    {
            ...
            else if (operation.m_generatedByType == "Gaia.Spawner")
            {
                ...
                {
                    if (spawners[spawnerIdx].m_spawnerID == operation.m_generatedByID)
                    {
