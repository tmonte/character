...
namespace Gaia
{
    ...
    public class Stamper : MonoBehaviour
    {
        ...
        public void LoadStamp()
        {
            ...
            if (!Gaia.Utils.CheckValidGaiaStampPath(m_stampPreviewImage))
            {
#if UNITY_EDITOR
        ...
        public void DeSerialiseJson(string json)
        {
            ...
            #if UNITY_EDITOR
            stamper.m_resources = Gaia.Utils.GetAsset(m_resourcesPath, typeof(Gaia.GaiaResource)) as Gaia.GaiaResource;
            if (m_imageMask != null)
            {
        ...
        public IEnumerator ApplyStamp()
        {
            ...
            for (int x = 0; x < newSmMaxX; x++)
            {
                ...
                for (int z = 0; z < newSmMaxZ; z++)
                {
                    ...
                    position = new Vector3(newSmXNU, 0f, newSmZNU);
                    position = RotatePointAroundPivot(position, pivot, negRotation);
                    ...
                    distance = Gaia.Utils.Math_Distance(origSmXNU, origSmZNU, pivot.x, pivot.z) * 2f;
        ...
        private void GeneratePreviewMesh()
        {
            if (m_previewMaterial == null)
            {
                ...
                {
                    if (Terrain.activeTerrain.terrainData.splatPrototypes.Length > 0)
                    {
                        Texture2D oldTex;
                        if (Terrain.activeTerrain.terrainData.splatPrototypes.Length == 4) //Defaults to cliff
                        {
                            oldTex = Terrain.activeTerrain.terrainData.splatPrototypes[3].texture;
                        }
                        else
                        {
