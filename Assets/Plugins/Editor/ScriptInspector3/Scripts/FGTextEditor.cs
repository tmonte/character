									...
									|| assembly.assemblyId == AssemblyDefinition.UnityAssembly.CSharpEditor)
								{
									...
									else if (declarations != null && declarations.Count > 0)
									{
										foreach (var decl in declarations)
										{
											...
											{
												codeViewPopupMenu.AddItem(
													new GUIContent("Go To Definition/" + fileName + " : " + (firstLeaf.token.Line + 1) + signature + " _"),
													false,
													d => GoToSymbolDeclaration((SymbolDeclaration) d),
													decl);
											}
		...
		if (Event.current.type == EventType.repaint)
		{
			...
			if (showLineNumbers)
			{
				...
				for (int i = fromLine; i < toLine; ++i)
				{
					...
					GUIContent lineNumberContent = new GUIContent(cache[i] ?? (cache[i] = (i + 1 + lineNumbersOffset).ToString()));
		...
		if (current.type == EventType.keyDown)
		{
			switch (current.keyCode)
			{
				...
				case KeyCode.KeypadEnter:
					...
					{
#if UNITY_EDITOR_OSX && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2 && !UNITY_4_3 && !UNITY_4_5 && !UNITY_4_6
						current.Use();
						//OpenAtCursor();
						EditorWindow.focusedWindow.SendEvent(EditorGUIUtility.CommandEvent("OpenAtCursor"));
						GUIUtility.ExitGUI();
...
//				nextCharacterIndex = textBuffer.ColumnToCharIndex(ref nextCaretColumn, nextCaretLine);
			...
			if (SISettings.smartSemicolonPlacement && lastTypedText == ";")
			{
				lastTypedText = null;
